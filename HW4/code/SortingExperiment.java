    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            if (nums.charAt(0) == 'r') {
                ...
                /*int rminimum = -100000000;
                int rmaximum = 100000000;*/

                int rminimum = -100;
                int rmaximum = 100;

                Random rand = new Random(1);    // 난수 인스턴스를 생성한다. 시드값 고정
            ...
            else if(nums.charAt(0) == 'S') { //정렬된 경우
                isRandom = true;    // 출력 형식 변경
                String[] nums_arg = nums.split(" ");
                int numsize = Integer.parseInt(nums_arg[1]);    // 총 갯수
                value = new int[numsize];    // 배열을 생성한다.
                for (int i = 0; i < value.length; i++) {
            ...
            else if(nums.charAt(0) == 'R') { //역정렬된 경우
                isRandom = true;    // 출력 형식 변경
                String[] nums_arg = nums.split(" ");
                int numsize = Integer.parseInt(nums_arg[1]);    // 총 갯수
                value = new int[numsize];    // 배열을 생성한다.
                for (int i = 0; i < value.length; i++) {
                    value[i] = value.length-i;
                }
            }
            else if (nums.charAt(0) == 'E') { //전부 같은 경우
                isRandom = true;    // 출력 형식 변경
                String[] nums_arg = nums.split(" ");
                int numsize = Integer.parseInt(nums_arg[1]);    // 총 갯수
                value = new int[numsize];    // 배열을 생성한다.
    ...
    static int partition(int[] value, int s, int e) {
        ...
        while(startOfAreaThird <= e-1) {
            if(value[startOfAreaThird] < pivot
                    /*|| (value[startOfAreaThird] == pivot && Math.random() < 0.5)*/ /* 중복요소 처리 */ ) {
                //  1구역으로 이동
